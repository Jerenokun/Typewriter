    ...
    if mode.get()==1 and s:
        ...
        if int(time_var)==0:
            ...
            with open("typewriter_data.json","r") as f:
                contents=j.load(f)
            contents['data'].append({'test_no':len(contents['data'])+1,'WPM':round((correct/test_time)*100),'ACC':round(0 if(correct+mistakes)==0 else(1 if mistakes==0 else correct/(correct+mistakes))*100),'Date':t.strftime('%m-%d-%Y')})
            v=contents['data'][-1]
...
gen_word()
word_box.configure(state='disabled')
entry = Entry(typing_tab, width = 80);entry.place(x=80,y=300)
...
reset_button=Button(typing_tab, text='reset', font='arial 10',bg='grey97',command=reset);reset_button.place(x=30,y=295)
